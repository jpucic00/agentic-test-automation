...
2. **The browser is in the loop.** The Planner and Healer don't guess selectors — they open the live staging app via Playwright MCP (accessibility tree, no screenshots) and *verify* selectors before committing them. The Generator deliberately works blind from the plan, because a focused code model writes better code with less context.
