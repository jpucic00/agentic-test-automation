...
- **Stale snapshots are trimmed from agent history.** Every MCP browser tool result embeds a full accessibility snapshot; over a long exploration the replayed history would drag dozens of stale ones into the model's long-context degradation zone. A history processor on the Planner/Healer keeps only the newest few snapshots verbatim (`SNAPSHOT_HISTORY_KEEP`, default 2) and stubs older ones — action confirmations and `browser_generate_locator` results are never trimmed.
