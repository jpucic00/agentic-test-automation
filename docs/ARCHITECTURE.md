...
- **Stale snapshots are trimmed from agent history — but milestone pages are anchored.** Every MCP browser tool result embeds a full accessibility snapshot; over a long exploration the replayed history would drag dozens of stale ones into the model's long-context degradation zone (high reasoning effort multiplies *exchanges per page*, so a purely chronological window of any size fills with frames of the current page and loses every other page). A history processor on the Planner/Healer therefore keeps two classes: the newest few snapshots (`SNAPSHOT_HISTORY_KEEP`, default 2 — "what's in front of me now") plus **anchors** — the latest snapshot of every page where a `browser_generate_locator` capture happened, deduped per (page URL, dialog-open) so modal and page states coexist while repeated fill→capture cycles collapse to one. Anchor count is bounded by the flow's pages, not by exchange count. Everything else is stubbed down to its action confirmation; `browser_generate_locator` results are never trimmed, so captured locators are unlosable. `ANCHOR_SNAPSHOTS=off` reverts to the pure chronological window.
