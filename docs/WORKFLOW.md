...
- **Working-memory housekeeping (Planner & Healer).** As the browser agents explore, stale page snapshots are trimmed from their conversation history: every *milestone* page — one where a locator was captured — keeps its latest snapshot (modal and page states separately), while transit frames are stubbed down to their action confirmations. Captured locators themselves are never trimmed. See the trimming bullet in [ARCHITECTURE.md](ARCHITECTURE.md) and `SNAPSHOT_HISTORY_KEEP` / `ANCHOR_SNAPSHOTS` in `.env.example`.
